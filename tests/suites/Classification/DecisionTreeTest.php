...
class DecisionTreeTest extends TestCase
{
    ...
    protected function generateBranch(&$rows, $output_dictionary_length, $max_depth = 100)
    {
        ...
        if ($row_num === count($rows)) {
            $sub_value = mt_rand(-10000, 10000) / 10000;
    ...
    {
        $data = [];
        for ($i = 0; ($i < 10000) and (count($data) < 30); $i++) {
        ...
        $num = count($data);
        for ($i = 0; ($i < 10000) and (count($data) - $num < 10); $i++) {
     ...
     * @param integer[] $input_types
     * @param double    $hard_limit
     * @param double    $soft_limit
     *
     ...
     * @return double[]
     */
    protected static function resolveSystem($system)
    {
        $matrix = [
                ...
                if (mt_rand(0, 2) == 0) {
                    ...
                    foreach ($output_dictionary as $tmp) {
                        if (mt_rand(0, 10) == 0) {
                            ...
                            $x = mt_rand(2, 998);
                            $system = [[0, mt_rand(1, 30)], [mt_rand($x + 1, 999) * 0.001, mt_rand(0, 10)], [$x * 0.001, 0]];
                        } else {
                            $x = mt_rand(2, 998);
                            $system = [[$x * 0.001, 0], [mt_rand($x + 1, 999) * 0.001, mt_rand(0, 10)], [1, mt_rand(1, 30)]];
                        }
                        $resolved = self::resolveSystem($system);
                    ...
                    $closure = function ($input, &$output) use ($resolved_s, $feature_number) {
                        foreach ($resolved_s as $num => &$resolved) {
                            ...
                            $output[$num] += $resolved[0] * $value + $resolved[1] * pow($value, 2) + $resolved[2];
                        }
                    };
